...
router.post("/routine", async (req, res) => {
  try {
    const { studentId, name, semesterSystem, routine } = req.body;
...
router.get("/demand", async (req, res) => {
  try {
    ...
    const students = await Student.find({}, { studentId: 1, name: 1, semesterSystem: 1, routine: 1 }).lean();
